...
postsServices.factory('Posts', function($http) {
	return {
	    get : function(sortField, sortDirection, limit, skip, count) {
	        return $http.get('http://localhost:4000/api/posts?select={"__v": 0, "comments": 0}&sort={"'+sortField+'":"'+sortDirection+'"}&limit='+limit+'&skip='+skip+'&count='+count);
	    },
	    ...
	    post : function (newTitle, newAuthor, newContent, newTags) {
	        ...
	        return $http.post('http://localhost:4000/api/posts', $.param(requestBody), 
	        	{
                	headers: {
